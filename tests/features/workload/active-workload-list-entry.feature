...
Feature: 워크로드 목록 페이지 진입
  ...
  Scenario: 대기중 워크로드 액션 버튼 활성화 상태 검증
    ...
    And 해당 워크로드의 웹터미널 버튼이 비활성화되어 있다
    And 해당 워크로드의 모니터링 버튼이 비활성화되어 있다
    And 해당 워크로드의 종료 버튼이 활성화되어 있다
  ...
  @regression @TESTUC-56
  Scenario: 활성화 워크로드 모니터링 조회
    Given 목록에 실행중인 워크로드가 있다
    And 해당 워크로드의 모니터링 버튼이 활성화되어 있다
    When 해당 워크로드의 모니터링 버튼을 클릭하여 모니터링 페이지로 이동한다
    Then CPU 사용량 영역에 차트가 표시된다
    And Memory 사용량 영역에 차트가 표시된다
    And GPU 사용률 영역에 차트가 표시된다
    And GPU 메모리 영역에 차트가 표시된다
