...
Feature: 활성화 워크로드 목록 페이지 진입
  ...
  # Scenario: 활성화 워크로드 목록 API 오류 시 에러 메시지 표시
  #   Given 활성화 워크로드 목록 API가 500 에러를 반환하도록 설정한다
  ...
  # @regression @edge-case
  # Scenario: 활성화 워크로드 목록이 비어있을 때 빈 목록 메시지 표시
  #   Given 활성화 워크로드 목록 API가 빈 목록을 반환하도록 설정한다
  #   When 활성화 워크로드 목록 페이지로 이동한다
  #   Then 워크로드 목록 페이지가 표시된다
  #   And 목록 페이지의 테이블이 표시된다
