...
import {
  GetPublicRegistryListImageSourceType,
  GetPublicRegistryListOrder,
 ...
 * ASC: public-image-001, public-image-002, ... (오름차순)
 * DESC: public-image-999, public-image-998, ... (내림차순)
 *
...
function generateImageDisplayName(
  ...
  const prefix = keyword || "public-image";
  ...
  if (sort === "IMAGE_NAME") {
    ...
    const sortableNum =
      order === GetPublicRegistryListOrder.ASC
        ? paddedIndex
...
function generateCreatedAt(
  ...
  if (sort === GetPublicRegistryListSort.CREATED_AT) {
...
function generateImageSourceType(
  ...
  return index % 2 === 0
    ? GetPublicRegistryListImageSourceType.SNAPSHOT
    : GetPublicRegistryListImageSourceType.EXTERNAL;
}

export const publicRegistryListOverrideHandlers = [
  getGetPublicRegistryListMockHandler(async (info) => {
    ...
    const sort =
      url.searchParams.get("sort") || GetPublicRegistryListSort.CREATED_AT;
    const order =
      url.searchParams.get("order") || GetPublicRegistryListOrder.DESC;
    const imageSourceType = url.searchParams.get(
      "imageSourceType",
    ) as GetPublicRegistryListImageSourceType | null;
    ...
    const mockResponse = getGetPublicRegistryListResponseMock();
    ...
    return getGetPublicRegistryListResponseMock({
      data: {
        totalSize,
