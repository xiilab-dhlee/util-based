...
const createColumnList =
  (): ResponsiveColumnType<ClusterNodeListResponse>[] => {
    return [
      {
        key: "index",
        dataIndex: "index",
      ...
      },
      {
        ...
        render: (nodeName: string, record: ClusterNodeListResponse) => {
          ...
          // migConfigState에 따라 버튼 비활성화 및 사유 표시
          const getDisabledReason = () => {
            switch (record.migConfigState) {
              case "PENDING":
                return "MIG 설정 진행 중입니다";
              case "NOT_SUPPORTED":
                return "MIG를 지원하지 않는 GPU입니다";
              default:
                return undefined;
            }
          };
