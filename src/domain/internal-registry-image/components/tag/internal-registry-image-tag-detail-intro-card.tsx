...
import { useGetInternalRegistryImageTag } from "@/domain/internal-registry-image/hooks/use-get-internal-registry-image-tag";
import { INTERNAL_REGISTRY_IMAGE_EVENTS } from "@/shared/constants/pubsub.constant";
import { usePublish } from "@/shared/hooks/use-pub-sub";
import { InternalRegistryImageTagSecurityCard } from "./internal-registry-image-tag-security-card";
 ...
 * 삭제 기능을 제공합니다.
 */
export function InternalRegistryImageTagDetailIntroCard() {
  const { id, tagId } = useParams();
  // Pub/Sub 시스템을 통한 이벤트 발행 훅
  const publish = usePublish();

  const { data } = useGetInternalRegistryImageTag({
    imageId: Number(id),
    tagId: Number(tagId),
  });
   ...
   */
  const handleDelete = () => {
    publish(INTERNAL_REGISTRY_IMAGE_EVENTS.sendDeleteImageTag, [Number(tagId)]);
  };
    ...
    <Container>
      ...
      <Body>
        ...
        <SecurityRow>
          ...
          <SecurityBody>
            <InternalRegistryImageTagSecurityCard
              level="CRITICAL"
