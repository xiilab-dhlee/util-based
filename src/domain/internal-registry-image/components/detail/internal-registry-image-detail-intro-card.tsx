...
import { useGetInternalRegistryImage } from "@/domain/internal-registry-image/hooks/use-get-internal-registry-image";
 ...
 * 삭제 기능을 제공합니다.
 */
export function InternalRegistryImageDetailIntroCard() {
  const { id } = useParams();
  // Pub/Sub 시스템을 통한 이벤트 발행 훅
  const publish = usePublish();

  const { data } = useGetInternalRegistryImage(Number(id));
   ...
   */
  const handleDelete = () => {
    publish(INTERNAL_REGISTRY_IMAGE_EVENTS.sendDeleteImage, [Number(id)]);
  };
