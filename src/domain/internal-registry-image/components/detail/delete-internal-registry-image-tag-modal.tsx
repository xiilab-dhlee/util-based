...
import { useDeleteInternalRegistryImageTag } from "@/domain/internal-registry-image/hooks/use-delete-internal-registry-image-tag";
import { openDeleteInternalRegistryImageTagModalAtom } from "@/domain/internal-registry-image/state/internal-registry-image.atom";
 ...
 */
export function DeleteInternalRegistryImageTagModal() {
  // useGlobalModal 훅을 사용하여 모달 상태 관리
  const { open, onOpen, onClose } = useGlobalModal(
    openDeleteInternalRegistryImageTagModalAtom,
  );

  // 삭제할 이미지 태그 목록
  const [deleteTagIds, setDeleteTagIds] = useState<number[]>([]);

  const deleteImageTag = useDeleteInternalRegistryImageTag();
