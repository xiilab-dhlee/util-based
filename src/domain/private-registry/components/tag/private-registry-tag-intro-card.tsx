...
interface PrivateRegistryTagIntroCardProps {
  imageId: number;
}
...
export function PrivateRegistryTagIntroCard({
  ...
  // 2. 태그 상세 조회
  const { data, isLoading } = useGetPrivateImageTagDetail(imageId, imageTagId, {
    harborImageName,
  });
  ...
  const handleDelete = () => {
    ...
    publish(PRIVATE_REGISTRY_EVENTS.sendDeleteImageTag, [data.imageTagId]);
  };
