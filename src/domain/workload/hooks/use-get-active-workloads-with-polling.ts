...
import { useGetActiveWorkloads } from "@/api/generated/workload/workload";
import { WORKLOAD_LIST_POLLING_INTERVAL } from "@/domain/workload/constants/workload.constant";

export interface UseGetActiveWorkloadsWithPollingParams {
  /** 워크스페이스 ID */
  workspaceId: number;
...
export function useGetActiveWorkloadsWithPolling({
  workspaceId,
  params,
  enabled = true,
}: UseGetActiveWorkloadsWithPollingParams) {
  return useGetActiveWorkloads(workspaceId, params, {
    query: {
