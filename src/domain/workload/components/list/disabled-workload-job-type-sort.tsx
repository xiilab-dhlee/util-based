...
import type { GetTerminatedWorkloadsWorkloadJobType } from "@/api/generated/astragoBackendAPIDocumentation.schemas";
...
interface DisabledWorkloadJobTypeSortProps {
  disabled?: boolean;
}

/**
 * 비활성화 워크로드 목록 페이지 작업 유형 정렬 컴포넌트
 *
 * 비활성화 워크로드 목록 페이지에서 작업 유형(INTERACTIVE, BATCH)을 선택하여
 * 해당 유형의 워크로드만 필터링할 수 있는 드롭다운 선택기를 제공합니다.
 *
 * @param disabled - 비활성화 여부
 * @returns 비활성화 워크로드 목록 페이지 작업 유형 정렬 컴포넌트
 */
export function DisabledWorkloadJobTypeSort({
  disabled,
   ...
   */
  const handleChange = (
    value: GetTerminatedWorkloadsWorkloadJobType | null,
  ) => {
    resetPage();
    setJobType(value);
  };
