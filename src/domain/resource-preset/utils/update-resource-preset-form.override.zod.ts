...
import {
  PresetGpuRequestGpuType as GpuType,
  ResourcePresetUpdateRequestNodeType as NodeType,
  ResourcePresetUpdateRequestWorkloadJobType as WorkloadJobType,
 ...
 * 리소스 프리셋 수정 폼 확장 스키마
 *
 * Orval 생성 스키마의 resource 필드는 min(1) 제약이 있음.
 * CPU/메모리는 0 허용, GPU count는 1 이상이며
 * 전체 합계는 최소 1개 이상(> 0)이어야 함.
 *
 * 또한 description 길이는 상수로 제한한다.
 */
export const updatePresetBodyExtended = z
  .object({
    presetName: z
    ...
    resource: z.object({
      cpu: z.object({
        requestCore: z.number().min(0),
      }),
      memory: z.object({
        requestByte: z.number().min(0),
      }),
      gpu: z
        .object({
          gpuType: z.nativeEnum(GpuType),
          detail: z.object({
            normal: z
              .object({
                requestCount: z
...
export type UpdatePresetBodyExtended = z.infer<typeof updatePresetBodyExtended>;
