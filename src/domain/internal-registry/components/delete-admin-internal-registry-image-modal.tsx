...
import { useDeleteAdminInternalRegistryImage } from "@/domain/internal-registry-image/hooks/use-delete-admin-internal-registry-image";
import { openDeleteAdminRegistryImageModalAtom } from "@/domain/internal-registry-image/state/internal-registry-image.atom";
...
export function DeleteAdminInternalRegistryImageModal() {
  // 모달 상태 관리
  ...
  const deleteInternalRegistryImage = useDeleteAdminInternalRegistryImage();
  ...
  const handleOk = () => {
    ...
    // 삭제 실행
    deleteInternalRegistryImage.mutate(
  ...
  useSubscribe(
    INTERNAL_REGISTRY_IMAGE_EVENTS.sendDeleteAdminRegistryImage,
    (payload: DeleteAdminInternalRegistryImagePayload) => {
      // 삭제할 컨테이너 이미지 정보 설정
  ...
  return (
    <Modal
      ...
      centered
      okButtonProps={{
        loading: deleteInternalRegistryImage.isPending,
      }}
    >
