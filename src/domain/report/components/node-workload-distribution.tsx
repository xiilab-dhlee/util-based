...
export function NodeWorkloadDistribution() {
  ...
  const displayData = useMemo(() => {
    const data = showAll ? nodeDistribution : nodeDistribution.slice(0, 5);
    return data.map((node, index) => ({
      ...node,
      id: node.nodeName,
      no: index + 1,
    }));
  }, [showAll, nodeDistribution]);
  ...
  return (
    <>
      <Header>
        <Title>2. 노드별 워크로드 작업 분배 정보</Title>
        <SwitchWrapper>
      ...
      >
        <CardBody>
          <TableWrapper>
            <CustomizedTable<NodeWorkloadDistributionType & { no: number }>
              columns={columns}
              data={displayData}
              headerFontSize={11}
              bodyFontSize={12}
