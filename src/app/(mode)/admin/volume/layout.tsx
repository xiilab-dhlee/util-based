...
/**
 * Admin Volume 페이지 공통 레이아웃
 *
 * User Volume 패턴을 따라 공통 레이아웃을 정의하고
 * children 슬롯을 통해 list/detail aside 컨텐츠를 렌더링합니다.
 *
 * - /admin/volume → VolumeListMain (자동 리다이렉트 로직 포함)
 * - /admin/volume/[id] → VolumeDetailMain (URL 파라미터 기반)
 */
export default function AdminVolumeLayout({ children }: PropsWithChildren) {
  ...
  const isVolumeListPage = pathname === ROUTES.ADMIN_VOLUME;

  const sortParams = parseVolumeSortValue(sort);

  // Admin에서는 workspaceId 없이 전체 볼륨 조회
  ...
  useEffect(() => {
    ...
    if (isVolumeListPage && content[0]) {
      router.push(ROUTES.ADMIN_VOLUME_DETAIL(content[0].volumeId));
    }
  }, [isVolumeListPage, content, router]);

  return (
    <>
      <PageHeader pageKey="admin.volume" />
      <ListPageMain>
        <ListPageBody>
