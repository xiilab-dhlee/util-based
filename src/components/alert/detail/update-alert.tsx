...
import { useGetAlert } from "@/hooks/alert/use-get-alert";
import {
...
import type { UpdateAlertPayload } from "@/types/alert/alert.type";
...
export function UpdateAlert() {
  const selectedAlert = useAtomValue(alertSelectedAtom);

  const { data } = useGetAlert(selectedAlert || "");
   ...
   *
   * 폼의 모든 입력 필드에서 데이터를 수집하고, UpdateAlertPayload 형태로 변환합니다.
   *
   * @returns UpdateAlertPayload 객체 또는 null (폼 참조가 없는 경우)
   */
  const createPayload = (): UpdateAlertPayload | null => {
                ...
                <AsideDetailArticleRow>
                  ...
                  <AsideDetailArticleRowItem>
                    <AsideDetailArticleColumn>
                      ...
                      <AsideDetailArticleValue className="truncate">
                        {data?.type && getAlertTypeLabel(data.type)}
                      </AsideDetailArticleValue>
                    </AsideDetailArticleColumn>
                  </AsideDetailArticleRowItem>
                </AsideDetailArticleRow>
